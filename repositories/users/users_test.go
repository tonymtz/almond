...
func TestUsersRepository_GetOneByGoogleId_NoErrorResult(t *testing.T) {
    db, mock, err := sqlmock.New()
    if err != nil {
        t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
    }
    ...
    // execute our method
    if user, err := myUsersRepository.GetOneByGoogleId(testUser.GoogleId); err != nil || user == nil {
        t.Errorf("error was not expected while updating stats: %s", err)
    }
...
func TestUsersRepository_GetOneByGoogleId_ErrorResult(t *testing.T) {
    db, mock, err := sqlmock.New()
    if err != nil {
        t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
    }
